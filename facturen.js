...
function renderFacturen(facturen) {
  ...
  facturen.forEach((factuur) => {
    ...
    const invoiceUrl = `factuur-preview.html?invoice=${encodeURIComponent(factuur.invoice_number)}`;
    const reminderUrl = `herinnering.html?invoice=${encodeURIComponent(factuur.invoice_number)}`;
